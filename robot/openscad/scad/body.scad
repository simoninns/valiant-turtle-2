...
module wheel_cutout_holes()
{
    ...
    move([0,0,9 - 1.5]) {
        move([59.5 + 2.5,-7.5,-8]) xrot(180) cyl(h=4,d=5.25);
        move([116.5 - 2.5,-7.5,-8]) xrot(180) cyl(h=4,d=5.25);

        // Back
        move([59.5 + 2.5,62,-8]) xrot(180) cyl(h=4,d=5.25);
        move([116.5 - 2.5,62,-8]) xrot(180) cyl(h=4,d=5.25);   
    }
}

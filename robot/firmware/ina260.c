...
void ina260_initialise() {
    ...
    if (ina260_read_manu_id() != 0x5449) debug_printf("Warning: INA260 did not respond with correct manufacturer ID!\r\n");
    if (ina260_read_die_id() != 0x2270) debug_printf("Warning: INA260 did not respond with correct die ID!\r\n");
}

// Return the current measurement in mA
float ina260_read_current() {
    uint8_t buf[2];
